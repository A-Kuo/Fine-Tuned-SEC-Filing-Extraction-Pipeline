...
**Status: target contract for a planned multi-service integration, not a deployed architecture.** This repository implements `FinDocAnalyzer` (`serving/api.py`) only. `TickerAgent` and `VizFramework` are separate, planned downstream services with no working integration in this repo. No webhook registration, ingestion, or rendering endpoint described below has been implemented or run against a real `TickerAgent`/`VizFramework` instance; the ports and payloads below are a design proposal for when those services exist, not evidence of a live three-service system.
