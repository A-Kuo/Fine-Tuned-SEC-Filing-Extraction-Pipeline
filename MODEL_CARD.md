...
| **Model Size** | ~85 MB (LoRA adapter only, fp16: 42M params × 2 bytes) |
...
**What's still open**: no trained LoRA adapter exists yet, so every real run above is base-weights-only — real per-field extraction accuracy from the fine-tuned model has not been measured.
...
Earlier documentation citing ~200M trainable parameters was incorrect because it assumed trainable embeddings and output-head weights that `create_lora_config()` never enables (no `modules_to_save` parameter is passed). A naive flat-dimension estimate (treating every projection as 4096×4096) also undercounts — the actual per-module breakdown is richer.
